...
export const todosFeature = createFeature({
  ...
  reducer: createReducer(
    initialState,
    on(TodosEvents.todoItemFailedCompleted, (s, a) =>
      adapter.updateOne({ id: a.payload.id, changes: { completed: false } }, s)
    ),
    on(TodosEvents.todoItemFailedCompleted, (s) => ({ ...s, hasError: true })),
    on(TodosEvents.todoItemCompleted, (s, a) =>
      adapter.updateOne({ id: a.payload.id, changes: { completed: true } }, s)
    ),
  ...
  extraSelectors: ({ selectTodosState }) => {
    ...
    const selectTodoList = createSelector(selectTodosState, (s) =>
      selectAll(s)
    );
